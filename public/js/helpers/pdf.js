/* =========================================================
 *  Generador reutilizable de reportes PDF (jsPDF vendoreado).
 *  API única para todas las pantallas que exportan tablas.
 * ========================================================= */
...
(function () {
  ...
  function dibujarPies(doc, anchoPag, altoPag) {
    ...
    for (let p = 1; p <= total; p++) {
      ...
      doc.text('Sistema Código Azul · Uso interno hospitalario',
        anchoPag / 2, altoPag - 6, { align: 'center' });
      doc.text(`Página ${p} de ${total}`, anchoPag - MARGEN_MM, altoPag - 6, { align: 'right' });
    }
  }

  /**
   * Genera y dispara la descarga directa de un PDF con formato institucional.
   *
   * @param {object} opts
   * @param {string}                                                    opts.titulo         - Título del reporte.
   * @param {string}                                                    opts.nombreArchivo  - Nombre del archivo a descargar.
   * @param {string}                                                   [opts.filtros]      - Descripción legible de filtros aplicados.
   * @param {Array<{t:string,v:string}>}                               [opts.kpis]         - Tarjetas KPI opcionales.
   * @param {Array<{key:string,label:string,w:number,align?:string}>}   opts.columnas       - Definición de columnas.
   * @param {Array<Record<string,any>>}                                 opts.filas          - Filas ya normalizadas.
   * @returns {boolean} true si generó y disparó descarga.
   */
  App.pdf.descargarTabla = function ({ titulo, nombreArchivo, filtros, kpis, columnas, filas }) {
    if (!window.jspdf || !window.jspdf.jsPDF) {
      App.ui.toast({ titulo: 'jsPDF no disponible — verificá la carga del script', tipo: 'error' });
      return false;
    }
    ...
    y = dibujarCabeceraTabla(doc, columnas, MARGEN_MM, y);
    dibujarFilas(doc, columnas, filas, altoPag, y);
