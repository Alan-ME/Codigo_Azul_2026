/* =========================================================
 *  Datos mock del sistema Código Azul
 *  Todo vive acá porque abrir por file:// no permite fetch.
 *  Se registra como App.data para que el resto lo consuma.
 * ========================================================= */

(function () {
  const App = window.App = window.App || {};

  // Utilidad para generar un avatar SVG con iniciales.
  ...
  const usuarios = [
    { id: 'u1', nombre: 'Julieta Molina',   usuario: 'jmolina',   email: 'j.molina@hospital.gob.ar',   rol: 'admin',     estado: 'activo', ultimoAcceso: '2026-08-20 07:12', areaId: null,  telefono: '+54 381 4123-889' },
    { id: 'u2', nombre: 'Rodrigo Peralta',  usuario: 'rperalta',  email: 'r.peralta@hospital.gob.ar',  rol: 'admin',     estado: 'activo', ultimoAcceso: '2026-08-19 22:41', areaId: null,  telefono: '+54 381 4551-002' },
    { id: 'u3', nombre: 'Camila Herrera',   usuario: 'cherrera',  email: 'c.herrera@hospital.gob.ar',  rol: 'enfermero', estado: 'activo', ultimoAcceso: '2026-08-20 08:03', areaId: 'a1',  telefono: '+54 381 4998-121' },
    { id: 'u4', nombre: 'Matías Sosa',      usuario: 'msosa',     email: 'm.sosa@hospital.gob.ar',     rol: 'enfermero', estado: 'activo', ultimoAcceso: '2026-08-20 06:58', areaId: 'a2',  telefono: '+54 381 4423-778' },
    { id: 'u5', nombre: 'Belén Aguirre',    usuario: 'baguirre',  email: 'b.aguirre@hospital.gob.ar',  rol: 'enfermero', estado: 'activo', ultimoAcceso: '2026-08-20 07:44', areaId: 'a3',  telefono: '+54 381 4802-014' },
    { id: 'u6', nombre: 'Nicolás Barrionuevo', usuario: 'nbarrionuevo', email: 'n.barrionuevo@hospital.gob.ar', rol: 'enfermero', estado: 'activo', ultimoAcceso: '2026-08-20 07:33', areaId: 'a6', telefono: '+54 381 4110-993' },
    { id: 'u7', nombre: 'Florencia Ríos',   usuario: 'frios',     email: 'f.rios@hospital.gob.ar',     rol: 'enfermero', estado: 'inactivo', ultimoAcceso: '2026-08-10 15:12', areaId: 'a4', telefono: '+54 381 4776-521' },
  ];
  usuarios.forEach(u => u.avatar = avatar(u.nombre));
  ...
  pacientes.forEach(p => p.avatar = avatar(p.nombre + ' ' + p.apellido));

  // ---------- Tipos y orígenes de llamado ----------
  const tiposLlamado = [
  ...
  const origenesLlamado = [
    { id:'cama',    nombre:'Botón de cama',      icono:'bed',       descripcion:'Botón físico ubicado en la baranda de la cama.' },
  ...
  // ---------- Utilidad: fechas ----------
  function fechaISO(offsetMin) {
    const d = new Date(Date.now() - offsetMin * 60000);
    return d.toISOString();
  }

  // ---------- Llamados activos ----------
  const llamadosActivos = [
  ...
  // ---------- Llamados históricos (generados con variedad) ----------
  function generarHistoricos() {
    const arr = [];
    ...
    let idc = 1;
    // 60 llamados repartidos en los últimos 30 días
    for (let i = 0; i < 60; i++) {
      ...
      const enfermerosAct = usuarios.filter(u => u.rol === 'enfermero' && u.estado === 'activo');
  ...
  const llamadosHistoricos = generarHistoricos();
